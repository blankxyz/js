AmCharts.AmMap = AmCharts.Class({
	...
	handleMouseMove: function(a) {
		...
		if (a = b.mapContainer) {
			...
			if (!isNaN(k)) {
				b.stopDrag();
				var k = Math.sqrt(Math.pow(k - e, 2) + Math.pow(h - f, 2)),
					t = b.previousDistance,
					h = Math.max(b.realWidth, b.realHeight);
				...
				if (!isNaN(t)) {
					var A = 5 * Math.abs(t - k) / h,
						...
						t = (b.zoomY() - t * A) * (h / g) + t * A;.1 < Math.abs(h - g) && (b.zoomTo(h, v, t, !0), b.mapWasPinched = !0, clearTimeout(b.pinchTO))
				}
	...
	selectObject: function(a) {
		...
		if (e = a.groupId)
			for (l = b.getGroupById(e), c = 0; c < l.length; c++)
				if (h = l[c], h.isOver = !1, e = h.displayObject)
		...
		if (c) {
			...
			for (e = 0; e < k; e++) {
				...
				if (0 !== f.childNodes.length || "" !== l || !1 !== p)
					...
					if (f.attributes &&
						f.attributes.length)
						for (l = 0; l < f.attributes.length; l++) m[f.attributes[l].name] = f.attributes[l].value;
...
AmCharts.SimpleButton = AmCharts.Class({
	...
	init: function(a, b, c, d, e, f, g, k, h, l, p) {
		...
		l.push(d);
		if (e = m.iconPath) f = m.iconSize, a = a.image(e, (b - f) / 2, (c - f) / 2, f, f), l.push(a), a.setAttr("opacity", p), a.mousedown(function() {
			m.handleDown()
		}).mouseup(function() {
			m.handleUp()
		}).mouseover(function() {
...
AmCharts.AreasProcessor = AmCharts.Class({
	...
	process: function(a) {
		...
		b.maxValueReal = h;
		b.minValueReal = l;
		for (e = 0; e < d; e++) f = a[e], isNaN(f.value) ? f.percents = void 0 : (f.percents = (f.value - l) /
		...
		for (e = 0; e < d; e++) {
			...
			var u = c.color,
				...
				M = c.preserveOriginalAttributes;
	...
	updateAllAreas: function() {
		...
		for (p = 0; p < g.length; p++) {
			...
			if (b.preserveOriginalAttributes) {
				if (h.customAttr)
					for (var m in h.customAttr) h.setAttr(m, h.customAttr[m])
...
AmCharts.ImagesProcessor = AmCharts.Class({
	...
	createImage: function(a, b) {
		...
		if (r) {
			...
			z.translate(v, l, g)
		} else q ? (isNaN(t) && (t = 10), isNaN(w) && (w = 10), z = d.image(q, 0, 0, t, w), z.node.setAttribute("preserveAspectRatio", "none"), z.setAttr("opacity", k), a.centeredReal && (v = isNaN(a.right) ? -t / 2 : t / 2, l = isNaN(a.bottom) ? -w / 2 : w / 2, z.translate(v, l))) : F && (z =
		...
		for (r = 0; r < b.length; r++) {
			...
			if (n.remainVisible || c.selectedObject == n.parentObject) {
				...
				var C = this.processCoordinates(n.x, c.realWidth),
					...
					P;
				if (0 < I)
					for (C = [], P = 0; P < I; P++) C.push(c.longitudeToCoordinate(J[P]));
				I = R.length;
				if (0 < I)
					for (H = [], P = 0; P < I; P++) H.push(c.latitudeToCoordinate(R[P]));
				if (0 < C.length) {
					AmCharts.dx = 0;
					AmCharts.dy = 0;
					...
					if ("none" != K) {
						var E, Q, S;
						if ("end" == K || "both" == K) y = C[C.length - 1], D = H[H.length -
					...
					n.fixedSize && J && (this.linesToResize.push({
						line: J,
						thickness: z
	...
	init: function(a, b) {
		if (this.enabled) {
			...
			if (this.showAsGradient) c = AmCharts.rect(b,
				...
				for (h = this.width / e, l = 0; l < e; l++) p = AmCharts.getColorFade(c, d, 1 * l / (e - 1)), p = AmCharts.rect(b, h, this.height, p, 1, this.borderThickness, this.borderColor, 1), AmCharts.setCN(a, p, "value-legend-color"), AmCharts.setCN(a, p, "value-legend-color-" + l), p.translate(h * l, g), f.push(p);
			d = c = 0;
			e = f.getBBox();
