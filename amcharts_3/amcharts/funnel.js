	...
	drawChart: function() {
		...
		if (AmCharts.ifArray(g))
			if (0 < this.realWidth && 0 < this.realHeight) {
				var d = Math.round(this.depth3D * Math.cos(this.angle * Math.PI / 180)),
				...
				0 < d && 0 > l && (this.neckHeight =
					this.neckWidth = 0, e ? f -= l / 2 : a -= l / 2);
				var p = m - n - p,
				...
				for (var z = p / 2 + n, F = (D - E) / ((B - I) / 2), A = 1, q = B / 2, B = (D - E) * (B + I) / 2 + I * E, G = a, L = 0, E = 0; E < g.length; E++) {
					var h = g[E],
						r;
					if (!0 !== h.hidden) {
						var u = [],
						...
						if ("height" == this.valueRepresents) k = D * h.percents / 100;
							...
							else if (!e && a + k > y || e && a - k < y) r = e ? Math.round(k + (a - k - y)) : Math.round(k - (a + k - y)), k = r / F, k = r + 2 * (-C - (J - k / 2) * r) / I
						}
						C = q - k / F;
						J = !1;
						!e && a + k > y || e && a - k < y ? (C = I / 2, u.push(z - q, z + q, z + C, z + C, z - C, z - C), e ? (r = k + (a - k - y), a < y && (r = 0), w.push(a, a, a - r, a - k, a - k, a - r, a)) : (r = k - (a + k - y), a > y && (r = 0), w.push(a, a, a + r, a + k, a + k, a + r, a)), J = !0) : (u.push(z - q, z + q, z + C, z - C), e ? w.push(a, a, a - k, a - k) : w.push(a, a, a + k, a + k));
						b.set();
						r = b.set();
						0 < d && 0 > l ? (w = C / q, u = -1, e || (u = 1), isNaN(A) &&
							(A = 0), u = (new AmCharts.Cuboid(b, 2 * q, u * k, d, l * A, h.color, h.alpha, this.outlineThickness, this.outlineColor, this.outlineAlpha, 90, 0, !1, 0, h.pattern, w)).set, u.translate(z - q, a - l / 2 * A), A *= w) : u = AmCharts.polygon(b, u, w, h.color, h.alpha, this.outlineThickness, this.outlineColor, this.outlineAlpha);
						...
						if (w = this.gradientRatio) {
							var v = [],
								t;
							for (t = 0; t <
								w.length; t++) v.push(AmCharts.adjustLuminosity(h.color, w[t]));
							0 < v.length && u.gradient("linearGradient", v);
							h.pattern && u.pattern(h.pattern)
						}
						0 < c && (this.chartCreated || r.setAttr("opacity", this.startAlpha));
...
AmCharts.Cuboid = AmCharts.Class({
	...
	draw: function() {
		...
		var q, G, L, h, r, u, w, k, C, J = f;
		E && (f = 0);
		var v, t, H, K, M = this.rotate;
		if (0 < Math.abs(e) || 0 < Math.abs(m))
			if (isNaN(y)) w = A, A = AmCharts.adjustLuminosity(F, -.2), A = AmCharts.adjustLuminosity(F, -.2), q = AmCharts.polygon(d, [0, e, b + e, b, 0], [0, m, m, 0, 0],
				...
				var N, O, P;
				M ? (N = c / 2, A = e / 2, P = c / 2, O = b + e / 2, t = Math.abs(c / 2), v = Math.abs(e / 2)) : (A = b / 2, N = m / 2, O = b / 2, P = c + m / 2 + 1, v = Math.abs(b / 2), t = Math.abs(m / 2));
				H = v * y;
				K = t * y;.1 < v && .1 < v && (q = AmCharts.circle(d, v, F, f, n, p, a, !1, t), q.translate(A, N));.1 < H && .1 < H && (w = AmCharts.circle(d, H, AmCharts.adjustLuminosity(F, .5), f, n, p, a, !1, K), w.translate(O, P))
			}
		f = J;
		1 > Math.abs(c) && (c = 0);
		1 > Math.abs(b) && (b = 0);
		!isNaN(y) && (0 < Math.abs(e) || 0 < Math.abs(m)) ? (x = [F], x = {
				fill: x,
				stroke: p,
				"stroke-width": n,
				"stroke-opacity": a,
			...
			0
		], [0, c], p, a, n, D) : 0 < I ? AmCharts.rect(d, b, c, x, f, n, p, a, I, B, D) : AmCharts.polygon(d, [0, 0, b, b, 0], [0, c, c, 0, 0], x, f, n, p, a, B, !1, D);
		b = isNaN(y) ? 0 > c ? [q, C, G, L, h, r, u, w, k, d] : [w, k, G, L, h, r, q, C, u, d] : M ? 0 < b ? [q, d, w] : [w, d, q] : 0 > c ? [q, d, w] : [w, d, q];
	...
	updateWidth: function() {
		...
		g.frame++;
		var d = g.totalFrames;
		g.frame <= d && (d = g.easing(0, g.frame, 1, g.rw - 1, d), g.width(d), setTimeout(function() {
