...
AmCharts.RecItem = AmCharts.Class({
	construct: function(a, b, c, d, e, f, g, h, k, l, m, n) {
		...
		"V" == Pa && (m = 0);
		var W;
		ca && (W = a.autoWrap && 0 === m ? AmCharts.wrappedText(z, c, t, r, q, ha, h, e, 0) : AmCharts.text(z, c, t, r, q, ha, h), ha = W.getBBox(), da = ha.width, Qa = ha.height);
		if ("H" == Pa) {
			...
			v += q / 2;
			"right" == Z ? (G += pa + ra + aa, v += ia, S ? (f || (v -= q / 2 + 3), G = G - (A + 4) - ma) : (G += A + 4 + u, v -= 2, G += ma)) : S ? (G += A + 4 - aa, f || (v -= q / 2 + 3), g && (G += pa, v += ia), G += ma) : (G += -A - u - 4 - 2 - aa, v -= 2, G -= ma);
...
AmCharts.RecFill = AmCharts.Class({
	construct: function(a, b, c, d) {
		...
		"V" == g ? (g = AmCharts.rect(k, a.viW, c, m, l), g.translate(e, b - h + f)) : (g = AmCharts.rect(k, c, a.viH, m, l), g.translate(b - h + e, f));
	...
	drawTitles: function() {
		...
		if (AmCharts.ifArray(a)) {
			...
			for (c = 0; c < a.length; c++) {
				var d = a[c],
					d = AmCharts.processObject(d, AmCharts.Title, this.theme);
				if (!1 !== d.enabled) {
					var e = d.color;
...
AmCharts.AmGraph = AmCharts.Class({
	...
	positionLabel: function(a, b, c, d, e, f) {
		...
		c.translate(a, b);
		h = c.getBBox();
		!this.showAllValueLabels && n && f && (h.height > k.columnHeight || h.width > k.columnWidth) && (c.remove(), c = !1);
		if (c && "serial" == this.chart.type)
			if (l) {
				if (0 > b || b >
					this.height) c.remove(), c = !1
	...
	createSerialGraph: function() {
		...
		if (!(S || "line" !=
				g && "smoothedLine" != g && "step" != g || (1 == d.length && "step" != g && "none" == this.bullet && (this.bullet = "round", this.resetBullet = !0), !Ia && void 0 == Ba || l))) {
			var Ra = Pa;
			Ra > f.max && (Ra = f.max);
			Ra < f.min && (Ra = f.min);
			f.logarithmic && (Ra = f.minReal);
			var Fa = f.getCoordinate(Ra),
				Hb = f.getCoordinate(f.max);
			r ? (da = n, Qa = Math.abs(Hb - Fa) + 1, qb = n, rb = Math.abs(fb - Fa) + 1, eb = pb = 0, f.reversed ? (W = 0, db = Fa) : (W = Fa, db = 0)) : (Qa = m, da = Math.abs(Hb - Fa) + 1, rb = m, qb = Math.abs(fb - Fa) + 1, db = W = 0, f.reversed ? (eb = p, pb = Fa) : eb = Fa + 1)
		}
		var Ga = Math.round;
		this.pmx = Ga(W);
		this.pmy = Ga(pb);
		...
		for (J = ca; J <= oa; J++) {
			...
			if (Aa) {
				...
				"start" == this.pointPosition &&
					(ka -= U / 2 + Math.floor(Sa * U / 2), qa = 0, Ea = Math.floor(U) + Math.floor(Sa * U));
				...
				if (!wb && V < la || wb && V < sa) v.isNegative = !0, Ia && (Ha = Ia), Wa && (Oa = Wa), void 0 != Ba && (za = Ba);
				Xa = !1;
				isNaN(V) || (l ? V > La ? (Ka && (Xa = !0), Ka = !1) : (Ka || (Xa = !0), Ka = !0) : v.isNegative = V < Pa ? !0 : !1, sa = V);
				void 0 != v.color && (Ha = v.color);
				v.fillColors && (Ha = v.fillColors);
				isNaN(v.alpha) || (Oa = v.alpha);
				switch (g) {
					case "line":
						if (isNaN(V)) z || (this.drawLineGraph(x, u, P, Q), x = [], u = [], P = [], Q = []);
						else {
							...
							if (Xa || void 0 != v.lineColor || void 0 != v.fillColors || !isNaN(v.dashLength)) this.drawLineGraph(x, u, P, Q), x = [F], u = [E], P = [], Q = [], !Ja || isNaN(K) || isNaN(M) || (P.push(M), Q.push(K)), l ? Ka ? (this.lineColorSwitched = ra, this.fillColorsSwitched = sb) : (this.lineColorSwitched = Ba, this.fillColorsSwitched = Ia) : (this.lineColorSwitched = v.lineColor, this.fillColorsSwitched = v.fillColors), this.dashLengthSwitched = v.dashLength;
							v.gap && (this.drawLineGraph(x, u, P, Q), x = [], u = [], P = [], Q = [])
						}
						break;
					case "smoothedLine":
						if (isNaN(V)) z || (this.drawSmoothedGraph(x, u, P, Q), x = [], u = [], P = [], Q = []);
						else {
							if (Math.abs(F - gb) >= ib || Math.abs(E - hb) >= ib) x.push(F), u.push(E), gb = F, hb = E;
							xa = F;
							ya = E;
					...
					case "step":
						if (!isNaN(V)) {
							r ? (isNaN(w) || (x.push(w), u.push(E - qa)), u.push(E - qa), x.push(F), u.push(E +
								Ea), x.push(F), !Ja || isNaN(K) || isNaN(M) || (isNaN(A) || (P.push(A), Q.push(K - qa)), P.push(M), Q.push(K - qa), P.push(M), Q.push(K + Ea))) : (isNaN(y) || (u.push(y), x.push(w), u.push(y), x.push(F - qa)), x.push(F - qa), u.push(E), x.push(F + Ea), u.push(E), !Ja || isNaN(K) || isNaN(M) || (isNaN(C) || (P.push(M - qa), Q.push(C)), P.push(M - qa), Q.push(K), P.push(M + Ea), Q.push(K)));
							w = F;
							y = E;
							A = M;
							C = K;
						...
						if (!isNaN(V)) {
							...
							if (!(V < Kb && la < Kb || V > Lb && la > Lb)) {
								...
								if (r) {
									...
									N < n && 0 < L && (na = new AmCharts.Cuboid(e, ea, L, D - a.d3x, I - a.d3y, Ha, Oa, Y, ta, T, ha, t, r, aa, Za, jb, ma), v.columnWidth = Math.abs(ea), v.columnHeight = Math.abs(L))
								} else {
									...
									R < m + Z * D && 0 < L && (this.showOnAxis && (N -= I / 2), na = new AmCharts.Cuboid(e, L, ea, D - a.d3x, I - a.d3y, Ha, Oa, Y, ta, this.lineAlpha, ha, t, r, aa, Za, jb, ma), v.columnHeight = Math.abs(ea), v.columnWidth = Math.abs(L))
								}
							}
					...
					case "candlestick":
						if (!isNaN(la) && !isNaN(V)) {
							...
							if (r) {
								...
								wb && V >= la && (Oa = 0);
								var na = new AmCharts.Cuboid(e, L, ea, D, I, Ha, Oa, Y, ta, T, ha, t, r, aa, Za, jb, ma),
									Ab, Bb;
					...
					if (this.end - this.start <= Pb || 0 === Pb) {
						...
						if ("regular" == X || "100%" == X) {
							...
							if (Sb) {
								...
								r ? (bb = E, ab = 0 > V ? F - Ub / 2 - 2 - cb - ob : F + Ub / 2 + 3 + cb + ob) : (ab = F, bb = 0 > V ? E + Vb / 2 + cb + ob : E - Vb / 2 - 3 - cb - ob);
								Ua.translate(ab, bb);
								f.totals[J] = Ua;
		...
		if ("line" == g || "step" == g || "smoothedLine" == g) "smoothedLine" == g ? this.drawSmoothedGraph(x, u, P, Q) : this.drawLineGraph(x, u, P, Q), S || this.launchAnimation();
...
AmCharts.ChartCursor = AmCharts.Class({
	...
	zoom: function(a, b, c, d) {
		...
		if (this.fullWidth) {
			...
			this.line = AmCharts.rect(this.container, b, c, this.cursorColor, this.cursorAlpha, 0);
				...
				if (c.mouseIsOver || A || C || this.forceShow)
					if (this.forceShow = !1, C) {
						...
						if (this.oneBalloonOnly) {
							q = Infinity;
							for (g = 0; g < B.length; g++) u = B[g], u.showBalloon && !u.hidden && u.balloonText && (w = r.axes[u.valueAxis.id].graphs[u.id], x = w.y, isNaN(x) || (t ? Math.abs(e - x) < q && (q = Math.abs(e - x), H = u) : Math.abs(f - x) < q && (q = Math.abs(f - x), H = u)));
						...
						if (a != this.previousIndex || H != this.previousMostCloseGraph)
							if (this.normalizeBulletSize(), this.destroyValueBalloons(), this.resizedBullets = [], B && this.valueBalloonsEnabled && z && c.balloon.enabled) {
								...
								for (g = 0; g < B.length; g++)
									if (u = B[g], x = NaN, (!this.oneBalloonOnly || u == H) && u.showBalloon && !u.hidden && u.balloonText && ("step" == u.type && "left" == u.stepDirection && (r = this.data[a + 1]), r)) {
						...
						b ? (h = {
							type: "changed"
						}, h.index = a, h.chart = this.chart, h.zooming = A, h.mostCloseGraph = H, h.position = t ? f : e, h.target = this, c.fire("changed", h), this.fire("changed", h), this.skipZoomDispatch = !1) : (this.skipZoomDispatch = !0, c.updateLegendValues(a));
...
AmCharts.wedge = function(a, b, c, d, e, f, g, h, k, l, m, n) {
	...
	0 < k && (I = " M" + t + "," + (x + k) + " L" + u + "," + (w + k), q ? (D || (I += " A" + (b - f) + "," + (k + c - g) + "," + (b + f) + "," + (k + c + g) + "," + u + "," + (w + k) + "," + y + "," + (A + k)), I += " L" + C + "," + (z + k), 0 < h && (D || (I += " B" + (b - h) + "," + (k + c - r) + "," + (b + h) + "," + (k + c + r) + "," + C + "," + (k + z) + "," + t + "," + (k + x)))) : (I += " A" + f + "," + g + ",0," + H + ",1," + y + "," + (A + k) + " L" + C + "," + (z + k), 0 < h && (I += " A" + h + "," + r + ",0," + H + ",0," + t + "," + (x + k))), I = a.path(I + " Z", void 0, void 0, "1000,1000").attr(B), d.push(I), I = a.path(" M" + t + "," + x + " L" + t + "," + (x + k) + " L" + u + "," + (w + k) + " L" + u + "," + w + " L" + t + "," + x + " Z", void 0, void 0, "1000,1000").attr(B), k = a.path(" M" + y + "," + A + " L" + y + "," + (A + k) + " L" + C + "," + (z + k) + " L" + C + "," + z + " L" + y + "," + A + " Z", void 0, void 0, "1000,1000").attr(B), d.push(I), d.push(k));
	q ? (D || (e = " A" + p(b - f) + "," + p(c - g) + "," + p(b + f) + "," + p(c + g) + "," + p(u) + "," + p(w) + "," + p(y) + "," + p(A)), f = " M" + p(t) + "," + p(x) + " L" + p(u) + "," + p(w) + e + " L" + p(C) + "," + p(z)) : f = " M" + t + "," + x + " L" + u + "," + w + (" A" + f + "," + g + ",0," + H + ",1," + y + "," + A) + " L" + C + "," + z;
...
AmCharts.AmDraw = AmCharts.Class({
	...
	addDefs: function(a) {
		if (AmCharts.hasSVG) {
			...
			c.appendChild(b);
			a.defs && (b = this.createSvgElement("defs"), c.appendChild(b), AmCharts.parseDefs(a.defs, b), this.defs = b)
		}
	}
});
...
AmCharts.AmLegend = AmCharts.Class({
	...
	drawLegend: function() {
		...
		"outside" == b ? (c = f.offsetWidth, e = f.offsetHeight, f.clientHeight && (c = f.clientWidth, e = f.clientHeight)) : (isNaN(c) || (f.style.width = c + "px"), f.className = "amChartsLegend " + a.classNamePrefix + "-legend-div");
		this.divWidth = c;
